...
func DeleteOneProduct(from, id_p, user_id string) {
	...
	case "Корзина":
		...
		models.CheckError(err)
	case "Избранное":
		_, err := Db.Exec(context.Background(), "delete from favour_table WHERE id_product = $1 and id_user = $2;", id_p, user_id)
		models.CheckError(err)
	}
}
func DeleteAllProducts(from string, user_id string) {
	switch from {
	case "Корзина":
...
func GetIdProductFromMessageCart(message string) string {
	und := "Номер в корзине: "
	id := ""
	word := ""
